...
class CommandHandler(ABC, Generic[C]):
    @abstractmethod
    def handle(self, command: C):
        pass


class QueryHandler(ABC, Generic[Q]):
    @abstractmethod
    def handle(self, query: Q):
        pass
...
class EventBus:
    ...
    def process_events(self, dispatcher: "Dispatcher"):
...
class MessageBus:
    ...
    def process_messages(self, dispatcher: "Dispatcher"):
...
class Dispatcher:
    def __init__(self, message_bus: MessageBus, event_bus: EventBus):
        self.message_bus = message_bus
        self.event_bus = event_bus
        self.command_handlers: Dict[type, CommandHandler] = {}
        self.query_handlers: Dict[type, QueryHandler] = {}
        self.event_handlers: Dict[type, List[EventHandler]] = {}

    def register_command_handler(
        self, command_class: type, handler_class: CommandHandler
    ):
        self.command_handlers[command_class] = handler_class

    def register_query_handler(self, query_class: type, handler_class: QueryHandler):
        self.query_handlers[query_class] = handler_class

    def register_event_handler(self, event_class: type, handler_class: EventHandler):
...
class UpdateUserCommand(Command):
    def __init__(self, user_id: str, new_data: Dict):
