...
class OrderCreatedEvent(Event):
    def __init__(self, order_id, user_id, items):
        super().__init__("OrderCreated", {"order_id": order_id, "user_id": user_id, "items": items})
...
class CreateOrderHandler:
    def handle(self, command):
        ...
        dispatcher.dispatch(OrderCreatedEvent(command.order_id, command.user_id, command.content["items"]))
...
class GetUserOrdersHandler:
    def handle(self, query):
        user_orders = [order for order in database.orders.values() if order["user_id"] == query.user_id]
...
# Multiple Event handlers for the same event type (OrderCreatedEvent)
class OrderCreatedEmailNotificationHandler:
    def handle(self, event):
        print(f"Sending email notification for Order ID: {event.content['order_id']} to User ID: {event.content['user_id']}")
...
class OrderCreatedInventoryHandler:
    def handle(self, event):
        print(f"Updating inventory for items: {event.content['items']} in Order ID: {event.content['order_id']}")

# Multiple Event handlers for the same event type (UserUpdatedEvent)
...
class UserUpdatedAuditHandler:
    def handle(self, event):
        print(f"Auditing update for User ID: {event.content['user_id']}. Data: {event.content['new_data']}")
...
dispatcher.register_event_handler("OrderCreated", OrderCreatedEmailNotificationHandler)
dispatcher.register_event_handler("OrderCreated", OrderCreatedAnalyticsHandler)
dispatcher.register_event_handler("OrderCreated", OrderCreatedInventoryHandler)

# Register multiple handlers for UserUpdatedEvent
dispatcher.register_event_handler("UserUpdated", UserUpdatedNotificationHandler)
dispatcher.register_event_handler("UserUpdated", UserUpdatedAuditHandler)
dispatcher.register_event_handler("UserUpdated", UserUpdatedCacheInvalidationHandler)

# Example usage
# Sending commands and queries to the Dispatcher
dispatcher.dispatch(CreateOrderCommand("1001", "u1", ["item1", "item2"]))
dispatcher.dispatch(UpdateUserCommand("u1", {"name": "Alice", "email": "alice@example.com"}))
